export default class Headers {
    constructor(parent) {
        this.parent = parent;
        ['dosh', 'fileh', 'opth', 'dirs', 'sects']
        .forEach(name => this.prop(name));
    }
    ...
    async read() {
        let buffer = await this.parent.readBuffer(0, 8192);
        this._dosh = await readDosHeader(buffer);
        this._sign = await readSignature(buffer, this._dosh.e_lfanew);
        this._fileh = await readFileHeader(buffer, this._dosh.e_lfanew + 4);
        this._opth = await readOptHeader(buffer, this._dosh.e_lfanew + 24, this._fileh.SizeOfOptionalHeader);
        this.dirOffset = this._dosh.e_lfanew + 24 + (this._opth.Magic == "10b" ? 96 : 112);
        this._dirs = await readDirectories(
            new DataView(buffer, this.dirOffset),
            this._opth.NumberOfRvaAndSizes);
        this.sectOffset = this.dirOffset + 8 * 16;
        this._sects = await readSections(new DataView(buffer, this.sectOffset), this._fileh.NumberOfSections);
    }
}
